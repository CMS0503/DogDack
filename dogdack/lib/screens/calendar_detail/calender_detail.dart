...
//////////////////////////////////////////시간 관련 변수//////////////////////////////////////////
  ...
  Future<Map<String, List<Object>>> getData() async {
    ...
    for (int i = 0; i < pointsResult.docs.length; i++) {
      if (i == 0) {
        day_hour_points[i] = pointsResult.docs[i]['totalTimeMin'].toDouble();
        week_hour_points[i] = pointsResult.docs[i]['totalTimeMin'].toDouble();
        day_distance_points[i] = pointsResult.docs[i]['distance'].toDouble();
        week_distance_points[i] = pointsResult.docs[i]['distance'].toDouble();
        day_goal_hour_points[i] = pointsResult.docs[i]['goal'].toDouble();
      } else {
        //이번주
        if (i < 7) {
          day_hour_points.add(pointsResult.docs[i]['totalTimeMin'].toDouble());
          day_distance_points.add(pointsResult.docs[i]['distance'].toDouble());
          day_goal_hour_points.add(pointsResult.docs[i]['goal'].toDouble());
        }
        if (i < 28) {
          week_hour_points.add(pointsResult.docs[i]['totalTimeMin'].toDouble());
          week_distance_points.add(pointsResult.docs[i]['distance'].toDouble());
          week_goal_hour_points.add(pointsResult.docs[i]['goal'].toDouble());
        }
      }
      // if(i>6 && i<14){
  ...
  Widget build(BuildContext context) {
    ...
    sum_day_walk_distance = sum_day_walk_distance ~/ day_distance_points.length;
    ...
    sum_week_walk_distance =
        sum_week_walk_distance ~/ week_distance_points.length;
    ...
    walk_goal_data = ((walk_hour_data / hour_goal_data) * 100).toInt();
    walk_goal_increment = (walk_goal_data - last_walk_goal_data).toInt();
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            CalWalkCardWidget(
                ...
                totalTimeMin: controller.startTime.toString()),
//            건강지수 타이틀  + 드롭다운 박스
